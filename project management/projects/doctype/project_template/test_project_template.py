...
def make_project_template(project_template_name, project_tasks=[]):
	if not frappe.db.exists("projTemplate", project_template_name):
		...
		doc = frappe.get_doc(dict(doctype="projTemplate", name=project_template_name))
		for task in project_tasks:
			doc.append("tasks", {"task": task.name})
		doc.insert()

	return frappe.get_doc("projTemplate", project_template_name)
